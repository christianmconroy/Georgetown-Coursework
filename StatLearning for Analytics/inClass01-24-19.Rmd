---
title: 'Analytics 512: In-Class 1/24/19'
output:
...
```{r}
# Given an X and the Coefficients, how can we make predictions about Y
model_prediction <- function(X, coeffs){
  y_hat <- coeffs[1] + X*coeffs[2]
  return(y_hat)
}

# How far off are you given your predictions?
...
# Evaluating error of sample data
coeffs <- c(8, 3.5)
y_hat <- model_prediction(x, coeffs)
prediction_error(y_hat, y)

# Create a grid of values of B1
beta1s <- data.frame(B0 = c(8, 8, 8, 8, 8, 8), B1 = c(3.5, 3.7, 3.9, 4.1, 4.3, 4.5))
# beta1s <- mapply(c, beta1s$B0, beta1s$B1, SIMPLIFY = FALSE)
# 
# mapply(model_prediction, x, beta1s)


# y_hat2 <- model_prediction(x, coeffs1)
# prediction_error(y_hat2, y)
...
for (i in 1:101) {
  y_hat <- model_prediction(x,c(b0,b1[i]))
  error[i] <- prediction_error(y_hat, y)
  return(error[i])
}
...
b <- for (i in 1:101) {
  y_hat_2 <- model_prediction(x,c(bb0,bb1[i]))
  error_2[i] <- prediction_error(y_hat_2, y)
  return(error[i])
}

# Good to double check and see if he posts any solution - Go back to chapter and figure out how to plot the minimization!
...
# We take original dataframe and take out what we just sampled and wrap that into a dataframe

# Now let's generate the bootstrap replicates of the data and compute the OLS
# estimate for each replicate (Like we're fitting a linear regression to new datasets)
bootStrapDataSets <- bootstrap(df, 500)
bootEstimates<-lapply(ols, bootStrapDataSets)

# BootStrapDataSets is a vector of two values, so not sure why this isn't working
