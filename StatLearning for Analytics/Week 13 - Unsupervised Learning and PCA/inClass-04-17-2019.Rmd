...
### Exercise
 The hyperparameter epsilon controls how "far apart" different points can be in order to be considered in the same cluster. Similarly, it controls the balance of which points are considered outliers. Explore changing epsilon from very small to very large and observe the impact on the clusters and outliers. Do  this for the both the linearly separable data and the spiral data. 
...
### TwentyNewsgroup
We'll use a pre-fit LDA model that has been trained for a long time. This is generated from news sources.

```{r, eval= FALSE}
#json <- topicmodels_json_ldavis(ap_lda, AssociatedPress)
data(TwentyNewsgroups, package = "LDAvis")
json <- createJSON(phi = TwentyNewsgroups$phi,
...
1. Use KMeans clustering with different values of K and plot the "average" face for each cluster. How do the faces change when you use different values of K?

```{r, eval= FALSE}
km.out <- kmeans(faces,10)
km.out

# olivetti_image(faces$V1) #8
# olivetti_image(faces$V32) #6
# olivetti_image(faces$V63) #10
# olivetti_image(faces$V343) #4
# olivetti_image(faces$V714) #2
# olivetti_image(faces$V995) #9

# To get the average image, we needed to use centers. So we'd typically do something like 

par(mfrow=c(2,5))
for i in seq(1,10)) {
  plot_digit(kmeans_res$centers[i,])
}
