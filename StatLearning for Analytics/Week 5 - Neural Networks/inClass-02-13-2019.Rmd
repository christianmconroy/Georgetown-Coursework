...
All of the above is minimizing the loss function. These are the value of loss after each iteration. It's going through the gradient descent process. 

If the loss doesn't change by some amount, we're probably close to the bottom and so we'll stop. So even if you specify higher iterations, it'll stop somewhere. 

2-2-1 means 2 inputs, 2 n-units (neurons?), 1 output

b -> h1 is bias on hidden value 
i1 -> h1 is input to hidden
h2 -> 0 is hidden unit 2 to output

So we're manually doing what we did with the Tensor Flow situation. 

Skip connections allow you to go straight from input to output. Linear regression is a special case - we want linout = true, size = 0, and skip = True. The weights will be equatable to the coefficient values in a linear regression 
...
```{r}
load("~/GeorgetownMPPMSFS/McCourtMPP/Semester 6 Spring 2019/StatLearning for Analytics/mnist68.RData")
images_df <- mnist68
```
