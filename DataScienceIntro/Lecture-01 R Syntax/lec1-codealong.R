...
# Use carrots to assign values
  ...
  # The format we know is on the right side.
...
# PASTE - Concatenates values together
  ...
  print(statement)
  # If we don't want space in between
  statement2 <- paste(tot_people, units1, "in", num_offices, units2, sep = " llama ")
  print(statement2)
...
# NUMERIC or ARITHMETIC operators
  ...
  4 / 2  #Division
  4 ** 2 #Exponent
  4 %% 2 #Modulus (The remainder) (Not to be confused with %*% for matrix multiplication)
...
# provides the average number of people served.
  mean_peopleoff =   
  statement3 <- paste("On average", tot_people/num_offices, units1, sep = " ")
  # Can use paste0 above if we don't want a space between each object and paste if we want spaces. Might be good for combining two vectors for example. So we really don't need sep = " " above because paste does it by default. 
  print(statement3)
  # print function is used here more as a formality to show that something is being done here. 
  
  paste(people, collapse = ",")
  # Prints all the stuff in people into one character object separated by commas
...
# a boolean that indicates if a statement is `TRUE`. Below, we compare two quantities: `y` and `x`
  ...
  y <- c(2,1)
  x==y
  # The above is really useful because it helps with extracting from a dataset like below. 
  ...
  # The below gives you the first value in x and then the third value in x
  x[1]
  x[3]
  
  # To extract all the values that are equal to eachother in two different vectors, then I can do the below to retreive values where the boolean is true. z means true here because it denotes x == y. Given a criteria, get me the values that fit that criteria. 
  x[z]
  y[z]
  
  # Can also do below, showing that we can do it directly instead of just creating an object. 
  x[x != y]
...
#Obtain the third through fifth cities
  cities[3:6]
  # The colon actually generates a vector here, as opposed to the comma, which does not, and hence the need not to concatanate 

##**EXERCISE**: How to get cities with populations of > 2 million
  
  cities[pop>2000000]
  #which returns index values whereas the noral condition above returns boolean 
  altcities <- cities
  altcities[pop > 2000000] <- "blah"
  altcities
  altcities[which(pop > 2000000)]  <- paste("blah", cities[which(pop > 2000000)])
  altcities
  # What we do above is place blah for all cities with more than 2000000
  # Which returns vectors of position which is far more efficient for replacing values 
...
# Matrices are better for commputation though!!!
...
#Being able to call a variable by name using the compact $ operator
    df$city
    df$population
# No one really uses attach() it appears, so just don't do it. 
...
#(2) extract the month from the date object using `format()`, which only requires the date object and the desired output format. In this case, the output format is "%m"
  ...
  table(df$month)
  # The leading 0 shows you that this is a character. 
  
  # Format takes the timestamp and extracts the month.
