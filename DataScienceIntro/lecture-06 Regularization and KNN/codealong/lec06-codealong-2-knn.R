...
#Load in data
  dir <- "~/GeorgetownMPPMSFS/McCourtMPP/Semester4MPP/DataScienceIntro/lecture-06 Regularization and KNN/data"
  setwd(dir)
...
#Cut down file to US
  ...
  ggplot(us_west, aes(x=lon, y=lat)) +
    geom_raster(aes(fill = ndvi)) +
    ggtitle("NDVI: October 2016") + 
    scale_fill_gradientn(limits = c(-1,1), colours = rev(terrain.colors(10)))
...
#KNN regression function - We'll use this to fill in the gray pixels 
...
#Comparisons
  ...
  # If the values are positive, large, non-zero values, use mape. 
  # RMSE is generalizable to anything. Typically for regression. 
