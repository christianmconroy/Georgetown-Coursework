...
setwd("/Users/chris/Documents/GeorgetownMPPMSFS/McCourtMPP/Semester4MPP/DataScienceIntro/lecture-03/ready")
  ...
  #clean up table
  # Look at below. We don't need to changee the ID names to be exact for matching every time!
  df <- merge(df, ref, by.x = "artistID", by.y = "id")
  df <- df[,c(2,4)]
  df$name <- as.character(df$name)
  df$flag <- 1
  
  # Flag means that the artist has been listened to by a user. 
  
  #Keep only top occurring artists
  top <- 500
  count <- aggregate(df$flag, by = list(df$name), FUN=length)
  # Remember that length is how we count the number of rows in the aggregation 
...
#Step 2: Set up data 
  ...
  # Data frame that only contains the artist and that will hold all the results. 
...
#Step3: Populate similarity matrix -- pairwise calculation
  ...
  #dimnames -> What each row and column will be named. We use the artists names here. 
