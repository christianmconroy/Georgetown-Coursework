...
#Set directory
setwd("/Users/chris/Documents/GeorgetownMPPMSFS/McCourtMPP/Semester4MPP/DataScienceIntro/lecture-09/data")
...
#Find elbow
  ...
  # When you initialize the algorithm, the points are randomly tossed to the space. We'd get the same results only if we set seed with something like set.seed(12) after print(i)
  for(i in seq(3,50,3)){
    print(i)
...
############################
  ...
  # Below we sample 10000 points at random.
  set.seed(120)
  nyc311.short <- nyc311[sample(1:nrow(nyc311), replace = FALSE, 10000), ]
  # Scale will provide the z-score. 
